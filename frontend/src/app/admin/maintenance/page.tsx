...
const DEFAULT_TASK_CONFIG = { label: "Maintenance", icon: "🛠️", color: "bg-gray-500" }
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-red-700">
                  {overdueTasks.length} task{overdueTasks.length > 1 ? 's' : ''} overdue: {overdueTasks.map(t => (taskTypeConfig[t.taskType]?.label || "Unknown").toLowerCase()).join(', ')}
                </p>
              </div>
            </div>
          </Card>
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {upcomingTasks.map((task) => {
                    ...
                    return (
                      ...
                      >
                        <div className={`p-4 ${taskConfig.color} text-white`}>
                          <div className="flex justify-between items-start">
                            <div>
                              <div className="text-2xl mb-1">{taskConfig.icon}</div>
                              <h3 className="font-bold text-lg">{taskConfig.label}</h3>
                        ...
                        <div className="flex justify-between items-start">
                          <div className="flex-1">
                            <div className="flex items-center gap-3 mb-2">
                              <div className={`p-2 ${taskConfig.color} text-white rounded-lg`}>
                                <span className="text-xl">{taskConfig.icon}</span>
                              </div>
                              <div>
                                <h3 className="font-semibold text-slate-900">
                                  {taskConfig.label} - {task.attendant?.name}
                                </h3>
